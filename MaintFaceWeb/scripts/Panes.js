...
var smallSizeMode = false;
// Set the layout
ApplyCustomLayout((document.body));
    ...
    if (window.innerWidth <= 640 || window.innerHeight <= 480) {
        if (smallSizeMode == false) {
            ...
            for (var i = 0; i < elems.length; i++) {
                elems[i].setAttribute("data-oldpane", "side-div");
                elems[i].setAttribute("data-oldisfront", elems[i].getAttribute("data-isfront"));
                MoveTabTo(elems[i--], mainDiv);
            }
            var elems = bottomDiv.getElementsByClassName("TabHeader");
            for (var i = 0; i < elems.length; i++) {
                elems[i].setAttribute("data-oldpane", "bottom-div");
                elems[i].setAttribute("data-oldisfront", elems[i].getAttribute("data-isfront"));
                MoveTabTo(elems[i--], mainDiv);
            }
        ...
        if (smallSizeMode == true) {
            ...
            for (var i = 0; i < elems.length; i++) {
                var oldPane = elems[i].getAttribute("data-oldpane");
                if (oldPane == null)
                    continue;
                MoveTabTo(elems[i--], document.getElementById(oldPane));
            }
            //bring the right tabs to the front again.
            var elems2 = document.getElementsByClassName("TabHeader");
            for (var i = 0; i < elems2.length; i++)
                if (elems2[i].getAttribute("data-oldisfront") == "true")
                    BringTabToFront(elems2[i].id);
...
function BringTabToFront(tabHeaderId) {
    ...
    for (var i = 0; i < tabHeader.parentElement.childNodes.length; i++) {
        var elem = tabHeader.parentElement.childNodes.item(i);
        if (elem.id == tabHeaderId)
            continue;
        if (elem.nodeType != 1)
            continue; //Text
    ...
    for (var i = 0; i < tabContent.parentElement.childNodes.length; i++) {
        var elem = tabContent.parentElement.childNodes.item(i);
...
function MoveTabTo(tabHeader, paneElem) {
    var tabContentId = tabHeader.getAttribute("data-tabcontentid");
    var tabContent = document.getElementById(tabContentId);
    ...
    for (var i = 0; i < oldTabHeaderParent.childNodes.length; i++) {
        if (oldTabHeaderParent.childNodes.item(i).nodeType != 1)
            continue;
        BringTabToFront(oldTabHeaderParent.childNodes.item(i).id);
        break;
    }
...
function HandleTabHeaderDrop(paneElem, event) {
