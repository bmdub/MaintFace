...
class GraphNode {
    EdgesUp: Array<Edge> = [];
    EdgesDown: Array<Edge> = [];
    X: number = 0;
    Y: number = 0;
    MaxStep: number = 0;
    IsRoot: boolean = false;
    IsLeaf: boolean = false;
	VarsOverridden: boolean;
	Script: boolean;
    LoopTag: boolean = false;
...
class Edge {
    ...
    BiggestThreadCount: number = 1;

    constructor(public Caller: GraphNode, public Callee: GraphNode, public AcceptableDuration: number) {
    ...
    CalculateBezierPoints() {
        ...
        // If flowing down, the edge is straight.
        ...
        // If flowing upward, the edge is bent
        else if (this.Callee.MaxStep < this.Caller.MaxStep) {
...
class Traffic {
    constructor(public StartTime: number, public DurationStat: Stat, public MaxThreads: number) {
    }
}

class DirectedGraph implements ICanvasPresentation {
    _inputHandler: InputHandler = null;
    _canvasCtx: CanvasRenderingContext2D = null;
	_canvasBuffer: HTMLCanvasElement = document.createElement('canvas');
    _canvasBufferCtx: CanvasRenderingContext2D = null;
    Nodes: { [index: string]: GraphNode } = {};
    NodeList: Array<GraphNode> = [];
    EdgeList: Array<Edge> = [];
    MaxPointsPerPipe: number = 7;
    MaxEdgeWidth: number = 3;
    StructureChanged: boolean = false;
    HeldNode: GraphNode = null;
	HeldNodeOffsetX: number = 0;
	HeldNodeOffsetY: number = 0;
    HoveredEdgeNames: Array<string> = [];
    hoveredEdgeStatAppends: Array<string> = [];
	...
	PrevScale: number = 0;
	NodeMovedSinceDraw: boolean = false;
	NodeHighlightChanged: boolean = false;
	Paused: boolean = false;
	Displayed: boolean = false;
	OnPauseChanged: (paused: boolean) => any;
        ...
        for (var i = 0; i < this.NodeList.length; i++) {
            if (this.MouseX >= this.NodeList[i].X - 20 && this.MouseX <= this.NodeList[i].X + 20 &&
                ...
                for (var j = 0; j < this.NodeList[i].EdgesDown.length; j++) {
                    ...
                    this.hoveredEdgeStatAppends.push(
						"Threads:" + this.NodeList[i].EdgesDown[j].Traffics[0].MaxThreads.toString());
                }
    ...
    TryAddToGrid(node: GraphNode, grid: { [index: string]: GraphNode }, cellDim: number) {
    ...
    RemoveFromGrid(node: GraphNode, grid: { [index: string]: GraphNode }, cellDim: number) {
        var x = Math.floor(node.X / cellDim);
        var y = node.MaxStep;
    ...
    FitToGrid(node: GraphNode, grid: { [index: string]: GraphNode }, cellDim: number) {
        ...
        if (this.TryAddToGrid(node, grid, cellDim) == false) {
            for (var o = 1; o < 1024; o++) {
                ...
                if (this.TryAddToGrid(node, grid, cellDim))
                    break;
                node.X -= o * cellDim;
                if (node.X - (o * cellDim) < 0) continue; //don't add to the left of the canvas
                node.X -= o * cellDim;
                if (this.TryAddToGrid(node, grid, cellDim))
                    break;
                node.X += o * cellDim;
    ...
    AvgNodePositionDown(node: GraphNode) {
        var accum = node.X;
        var count = 1;
    ...
    AvgNodePositionUp(node: GraphNode) {
        var accum = node.X / 3;
        var count = 1 / 3;
    ...
    DiscoverMaxNodeDepth(node: GraphNode, startIndex: number) {
    ...
    Build(graph: HTMLCanvasElement, ctx: CanvasRenderingContext2D) {
        // Find the root nodes
        var rootNodes: Array<GraphNode> = [];
        for (var i = 0; i < this.NodeList.length; i++)
        ...
        var grid: { [index: string]: GraphNode } = {};
        ...
        else { // Pad the edges and align left.
            for (var i = 0; i < this.NodeList.length; i++) {
    ...
    AddTraffic(callerName: string,
		callerVarsOverridden: boolean,
		callerScript: boolean,
		calleeName: string,
		calleeVarsOverridden: boolean,
		calleeScript: boolean,
		isRoot: boolean,
		isLeaf: boolean,
		traffic: Traffic) {
    ...
    GetEdge(callerName: string, calleeName: string) {
    ...
    Draw() {
		...
		if (graphChanged) {
			...
			for (var i = 0; i < this.EdgeList.length; i++) {
				...
				this.DrawEdge(this.EdgeList[i].Caller.X, this.EdgeList[i].Caller.Y,
					bezierPoints.cx, bezierPoints.cy, bezierPoints.cx2, bezierPoints.cy2,
					this.EdgeList[i].Callee.X, this.EdgeList[i].Callee.Y,
					this.EdgeList[i].BiggestThreadCount, dotRadius, this._canvasBufferCtx);
			}
			...
			for (var i = 0; i < this.NodeList.length; i++) {
				...
				if (this.NodeList[i].IsRoot) {
					var grd = this._canvasBufferCtx.createRadialGradient(
						this.NodeList[i].X, this.NodeList[i].Y + 5, 5, this.NodeList[i].X, this.NodeList[i].Y + 5, 25);
					...
					var grd = this._canvasBufferCtx.createRadialGradient(
						this.NodeList[i].X, this.NodeList[i].Y, 5, this.NodeList[i].X, this.NodeList[i].Y, 25);
		...
		if (!this.Paused) {
			...
			for (var i = 0; i < this.EdgeList.length; i++) {
				...
				for (var t = 0; t < this.EdgeList[i].Traffics.length; t++) {
					...
					var bezierPoints = this.EdgeList[i].CalculateBezierPoints();
					this.DrawTraffic(this.EdgeList[i].Caller.X, this.EdgeList[i].Caller.Y,
						bezierPoints.cx, bezierPoints.cy, bezierPoints.cx2, bezierPoints.cy2,
						this.EdgeList[i].Callee.X, this.EdgeList[i].Callee.Y,
						this.EdgeList[i].Traffics[t].DurationStat.Samples,
						trafficTime / travelTime,
						this.EdgeList[i].Traffics[t].MaxThreads,
						dotRadius,
						this._canvasCtx);
				}
			}
    ...
    DrawEdge(x1, y1, cx, cy, cx2, cy2, x2, y2, threadCt, radius, ctx: CanvasRenderingContext2D) {
    ...
    DrawTraffic(x1, y1, cx, cy, cx2, cy2, x2, y2, pointCount, pctDone, threadCt, radius, ctx: CanvasRenderingContext2D) {
        var girth = Math.min(this.MaxEdgeWidth, threadCt);
