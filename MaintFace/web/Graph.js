...
var GraphLine = (function () {
    function GraphLine(Series, Color) {
        this.Series = Series;
        this.Color = Color;
    }
    return GraphLine;
...
var Graph = (function () {
    function Graph(_canvas) {
        this._canvas = _canvas;
        this._inputHandler = null;
        this._canvasCtx = null;
        this._graphLine = [];
        this._graphStepSize = 10;
        this._verticalPadding = 60;
        this._closeButtonWidth = 16;
        this.MouseX = 0;
        this.MouseY = 0;
    ...
    Graph.prototype.AddSeries = function (statSeries) {
        ...
        for (;;) {
            ...
            if (this._graphLine.length >= this._colorTable.length)
                break;
            var found = false;
    ...
    Graph.prototype.RemoveSeries = function (statSeries) {
        for (var s = 0; s < this._graphLine.length; s++)
            if (this._graphLine[s].Series == statSeries) {
    ...
    Graph.prototype.ScreenToWorld = function (x, y) {
        x += this.CameraX;
        //x /= this.Scale;
        return { x: x, y: y };
    };
    Graph.prototype.WorldToScreen = function (x, y) {
        //x *= this.Scale;
        x -= this.CameraX;
        return { x: x, y: y };
    };
    Graph.prototype.HandleMouseLeave = function () {
    };
    Graph.prototype.HandleMouse = function (x, y, isDown, clicked) {
        if (isDown)
    ...
    Graph.prototype.LineUpPointToScreen = function (worldX, worldY, screenX, screenY) {
    ...
    Graph.prototype.DrawText = function (text, x, y, drawBox) {
