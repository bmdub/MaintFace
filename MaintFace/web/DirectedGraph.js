...
var Edge = (function () {
    function Edge(Caller, Callee, AcceptableDuration) {
        this.Caller = Caller;
        this.Callee = Callee;
    ...
    Edge.prototype.CalculateBezierPoints = function () {
        var cx, cy, cx2, cy2;
...
var Traffic = (function () {
    function Traffic(StartTime, DurationStat, MaxThreads) {
...
var DirectedGraph = (function () {
    function DirectedGraph(_canvas) {
    ...
    DirectedGraph.prototype.SetDisplayed = function (displayed) {
    ...
    DirectedGraph.prototype.SetPaused = function (paused) {
    ...
    DirectedGraph.prototype.ScreenToWorld = function (x, y) {
        x += this.CameraX;
        y += this.CameraY;
        x /= this.Scale;
        y /= this.Scale;
    ...
    DirectedGraph.prototype.WorldToScreen = function (x, y) {
        x *= this.Scale;
        y *= this.Scale;
        x -= this.CameraX;
        y -= this.CameraY;
    ...
    DirectedGraph.prototype.HandleMouseLeave = function () {
        this.HoveredEdgeNames.length = 0;
        this.hoveredEdgeStatAppends.length = 0;
        this.HoveredNodeName = null;
    };
    DirectedGraph.prototype.HandleMouse = function (x, y, isDown, clicked) {
    ...
    DirectedGraph.prototype.LineUpPointToScreen = function (worldX, worldY, screenX, screenY) {
    ...
    DirectedGraph.prototype.IncrementScale = function (delta, screenX, screenY) {
        this.ChangeScale(this.Scale + delta, screenX, screenY);
    };
    DirectedGraph.prototype.ChangeScale = function (scale, screenX, screenY) {
        var worldCoords = this.ScreenToWorld(screenX, screenY);
        this.Scale = scale;
        this.LineUpPointToScreen(worldCoords.x, worldCoords.y, screenX, screenY);
    };
    DirectedGraph.prototype.TryAddToGrid = function (node, grid, cellDim) {
    ...
    DirectedGraph.prototype.RemoveFromGrid = function (node, grid, cellDim) {
        var x = Math.floor(node.X / cellDim);
        var y = node.MaxStep;
        var key = x.toString() + "." + y.toString();
        if (grid[key] == node) {
    ...
    DirectedGraph.prototype.FitToGrid = function (node, grid, cellDim) {
    ...
    DirectedGraph.prototype.AvgNodePositionDown = function (node) {
        var accum = node.X;
        var count = 1;
    ...
    DirectedGraph.prototype.AvgNodePositionUp = function (node) {
        var accum = node.X / 3;
        var count = 1 / 3;
    ...
    DirectedGraph.prototype.DiscoverMaxNodeDepth = function (node, startIndex) {
    ...
    //    return { min, max };
    //}
    DirectedGraph.prototype.Build = function (graph, ctx) {
        // Find the root nodes
        var rootNodes = [];
        for (var i = 0; i < this.NodeList.length; i++)
    ...
    DirectedGraph.prototype.Hash = function (str) {
    ...
    DirectedGraph.prototype.AddTraffic = function (callerName, callerVarsOverridden, callerScript, calleeName, calleeVarsOverridden, calleeScript, isRoot, isLeaf, traffic) {
    ...
    DirectedGraph.prototype.GetEdge = function (callerName, calleeName) {
    ...
    DirectedGraph.prototype.Draw = function () {
        var dotRadius = 3;
    ...
    DirectedGraph.prototype.DrawEdge = function (x1, y1, cx, cy, cx2, cy2, x2, y2, threadCt, radius, ctx) {
    ...
    DirectedGraph.prototype.DrawTraffic = function (x1, y1, cx, cy, cx2, cy2, x2, y2, pointCount, pctDone, threadCt, radius, ctx) {
        var girth = Math.min(this.MaxEdgeWidth, threadCt);
