...
namespace SimpleTestApp
{
	class Program
	{
		static void Main(string[] args)
		{
			...
			MaintFace.Start("My App", "http://localhost:12121/MyApp/", MaintFaceOptions.EnableButtons | MaintFaceOptions.EnableCommands, AuthenticationSchemes.Basic);
			...
			// Remove the default perf counters.
			//MaintFace.PerfStats.Clear();

			// Add a perf counter.
			MaintFace.PerfStats["Handle Count"].PerfCounter =
				new PerformanceCounter("Process", "Handle Count", Process.GetCurrentProcess().ProcessName);
			MaintFace.PerfStats["% Processor Time"].ExpectedMaxNumber = 5;
